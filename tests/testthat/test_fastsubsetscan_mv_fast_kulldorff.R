...
test_that("fast_kulldorff_priority: works as intended", {
  ...
  rr <- c(1.2, 1.3)
  expected <- c(fk_priority_fun_poisson(1, 4, 1.2) + 
                  fk_priority_fun_poisson(2, 3, 1.3),
                fk_priority_fun_poisson(3, 2, 1.2) +
                  fk_priority_fun_poisson(4, 1, 1.3))
  res <- fast_kulldorff_priority(ags, rr, fk_priority_fun_poisson)
