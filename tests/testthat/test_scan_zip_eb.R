...
#   out1_actual <- score_eb_zip(in1$y, in1$mu, in1$p)
...
#   out2_actual <- score_eb_zip(in2$y, in2$mu, in2$p, 1e-3)
...
#   
#   actual1 <- scan_eb_zip_cpp(in1$counts, in1$baselines, in1$probs, 
...
#   
#   actual2 <- scan_eb_zip_cpp(in2$counts, in2$baselines, in2$probs, 
...
#     3, 
#     1,  
#     4 / (3 + 2 * (1 - est_eb_zip_zeroindic(2, 0.2, 1))),
#     3 / (1 + 0.5 * (1 - est_eb_zip_zeroindic(0.5, 0.1, 1))),
#     21 / (4 + 2 * (1 - est_eb_zip_zeroindic(
#       2, 0.2, 21 / (4 + 2 * (1 - est_eb_zip_zeroindic(2, 0.2, 1)))))))
#   
#   expected2_score <- c(
#     # Duration = 1
...
#   in1$zone_lengths = unlist(lapply(in1$zones, length))
#   
#   actual1 <- scan_eb_zip_cpp_max(in1$counts, in1$baselines, in1$probs, 
#                              in1$zones_flat - 1, in1$zone_lengths)
...
#   in2$zone_lengths = unlist(lapply(in2$zones, length))
#   
#   actual2 <- scan_eb_zip_cpp_max(in2$counts, in2$baselines, in2$probs, 
#                              in2$zones_flat - 1, in2$zone_lengths)
#   
#   expected2_relrisk <- 21 / (4 + 2 * (1 - est_eb_zip_zeroindic(
#     2, 0.2, 21 / (4 + 2 * (1 - est_eb_zip_zeroindic(2, 0.2, 1))))))
#   expected2_score <- 
