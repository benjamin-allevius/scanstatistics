...
3. For each cluster, compute a statistic based on both the observed and tfile:///media/bekj9674/suhome/Downloads/scanstatistics/data/NM_geo.rda
file:///media/bekj9674/suhome/Downloads/scanstatistics/data/NM_map.rda
file:///media/bekj9674/suhome/Downloads/scanstatistics/data/NM_popcas.rda
he 
...
- __`scan_eb_poisson`__: computes an \emph{expectation-based} scan statistic for 
  data following a Poisson distribution.
- __`scan_pb_poisson`__: computes a \emph{population-based} scan statistic for 
  data following a Poisson distribution.
- __`scan_eb_negbin`__: computes an \emph{expectation-based} scan statistic for 
  data following a negative binomial distribution.
- __`scan_eb_zip`__: computes an \emph{expectation-based} scan statistic for 
  data following a zero-inflated Poisson distribution.
...
map using the data table `NM_map` supplied by the _scanstatistics_ package:
```{r, eval=FALSE}
...
The _scanstatistics_ package provides the function `scan_poisson`, which is an 
expectation-based[^1] scan statistic for univariate Poisson-distributed data 
proposed by @Neill2005. It also provides the population-based scan statistic 
[@Kulldorff2001].
...
The first argument to `scan_eb_poisson` should be a matrix of integers, with 
columns representing locations and rows representing time intervals (ordered
chronologically from the earliest interval in the first row to the most recent
in the last). In this example we would like to detect a potential cluster of 
...
section: an anomaly that occurs in a space-time window $W$ will have the effect
of increasing the expected value of the counts in that window by a factor 
$q_W>1$ in comparison to what was predicted. This factor $q_W$ is the same for 
all locations in $W$ and constant over time. In the second version of the scan 
statistic, the factor $q_W$ increases monotonically over time. The null and 
alternative hypotheses are otherwise as for the Poisson scan statistic, except
that the negative binomial distribution is used instead. Further, the scan 
statistic is calculated using the score and Fisher information rather than a
likelihood ratio.
...
the expected value $\mu$ of the Poisson component and the probability $p$ of a
structural zero.

#### Theoretical motivation
The ZIP scan statistic makes a similar assumtion regarding outbreaks as the 
Poisson scan statistic does: an anomaly that occurs in a space-time window $W$ 
will have the effect of increasing the Poisson expected value parameter of the 
counts in that window by a factor $q_W>1$ in comparison to what was predicted. 
This factor $q_W$ is the same for all locations in $W$ and constant over the 
duration of the anomaly. For all windows $W$ considered, $q_W$ is estimated 
using the EM algorithm and a likelihood ratio statistic is computed. The scan
statistic is the maximum of these statistics over all windows $W$.
...
Carlo replications to obtain a $p$-value:
