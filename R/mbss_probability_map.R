...
#' 
#' Compute the probability map for all event types \eqn{E_k} and all locations 
#' \eqn{s_i}. The probability maps is, for each such location and event type, 
#' the sum of the posterior probabilities of that event type, over all regions 
#' \eqn{S} containing the location \eqn{s_i}.
...
#'    columns \code{region} and \code{event} (keyed in that order), and the 
#'    column \code{posterior_logprob}, which contains the posterior 
#'    log-probabilities log(P\eqn{(H_1(S,E_k)|D)}) for each region and event. 
