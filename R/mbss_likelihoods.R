...
#' considering events of duration up to \eqn{W_{\max}}.
...
#' Computes log-likelihood for each space-duration region and event type.
#'
#' Computes the log-likelihoods log(P\eqn{(D|H_1(S,E_k), W)})
#' given that an event of type \eqn{E_k} 
#' with duration \eqn{W} occurs in spatial region \eqn{S},
