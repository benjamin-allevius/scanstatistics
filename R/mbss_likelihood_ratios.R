...
#' @param llrs_given_effects A \code{data.table} with columns \code{location, 
#'    event, duration, stream, llr, effect_logprob}. For a given row (i.e. 
#'    location-duration-stream-event combination), the column \code{llr} 
#'    contains the log-likelihood ratio corresponding to that unit of 
#'    observation, assuming an event of that type occurs with a given effect. 
#'    The column \code{effect_logprob} contains the log-probability of that 
#'    specific event effect for that specific unit of observation.
#' @return A \code{data.table} with columns 
#'    \code{location, event, duration, stream, llr}; the first four being key 
#'    columns (in that order) and the last containg the log-likelihood.
...
#' event type \eqn{E_k}.
#' @param spacetime_llrs A \code{data.table} with columns \code{region, event, 
#'    duration, llr}. The first three columns are key columns in the given 
#'    order, and column \code{llr} contains the space-duration log-likelihood 
#'    ratios.
#' @param dur_given_event_logprobs A numeric matrix or data frame containing the 
#'    conditonal log-probabilities of the duration of an event given the event 
#'    type. Rows correspond to event durations and columns correspond to events, 
#'    so element \eqn{(i,j)} gives the log of the probability that the duration 
#'    is \eqn{i}, given that the event is of type \eqn{j}.
